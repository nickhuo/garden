...
> **位置（canonical）：** `/Users/nickhuo/Core/brain/03_Resources/.Codex/skills/daily-curation-digest/SKILL.md`
> 现已是 `03_Resources/.Codex/skills/` 下的**项目级 skill**，Codex 在 workspace=`03_Resources` 时自动发现，按 `name` / 触发语自动加载（无需按路径 `Read`）。原 `digest/curation/SKILL.md` 已迁入此处。
