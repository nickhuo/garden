...
This file overrides the root `Core/AGENTS.md` for everything inside `03_Resources/`.

## Vault root vs project root (READ THIS)

**Vault root = `brain/`** (one level above `03_Resources/`). **Project root = `03_Resources/`** (where this AGENTS.md lives).

- Filesystem operations from Codex skills are **project-relative**: `wiki/sources/X.md` resolves to `03_Resources/wiki/sources/X.md`.
...
├── AGENTS.md          (this file — vault-specific deltas)
...
This vault is designed for the `Codex-obsidian` plugin (github.com/AgriciDaniel/Codex-obsidian). If installed, its skills handle ingest/query/lint/save/autoresearch automatically. If not installed, follow `WIKI.md` directly.

**Install (run in Codex CLI on Nick's machine):**

```
Codex plugin marketplace add AgriciDaniel/Codex-obsidian
```
...
- **Access:** query via the **`gbrain` MCP server** (stdio, registered at user scope → available in
  every Codex session). Tools cover hybrid/keyword search, page reads, `backlinks`, `graph`, and
...
- **Don't:** run `gbrain skillpack scaffold` into this repo (it would collide with the existing
  llm-wiki / Codex-obsidian / gstack skills), and don't restructure the wiki to gbrain's schema.
...
the Codex session restarts** (the CLI's own `gbrain query` sees it immediately). After an ingest,
...
```bash
Codex mcp add gbrain --scope user \
...
Codex marks the server **"✗ Failed to connect"** (the 85 tools never load). Do **not**
substitute `GBRAIN_OFFLINE=1` — it also blocks the ZeroEntropy embedding calls that query-time
search depends on. After registering, **restart the Codex session** so the tools load.

### Orchestration with Codex-obsidian operations
...
- Plugin: https://github.com/AgriciDaniel/Codex-obsidian
